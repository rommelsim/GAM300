...
namespace AeonCore
{
	...
	static uint32_t ShaderDataTypeSize(ShaderDataType type)
	{
		...
		case ShaderDataType::Bool:		return 1;		// might be 1 b or 4 b
	...
	public:
		inline uint32_t GetElementCount()
		{
			...
			std::cout << "Unknown Shader Element count\n";
			return 0;
		}
	};
	...
	public:
		...
		//virtual void UpdateBufferStore(float* vertices, uint32_t size) = 0;
		virtual void UpdateBufferStore(void* vertices, uint32_t size) = 0;

		// static VertexBuffer* Create(float* vertices, uint32_t size);
		static VertexBuffer* Create(void* vertices, uint32_t size);
		static VertexBuffer* Create(uint32_t size);
		static VertexBuffer* Create(std::vector<glm::mat4> _matrices);
	};
	...
	public:
		...
		virtual void Bind() const = 0;
		virtual void UnBind() const = 0;
		virtual void SetData(uint32_t offset, uint32_t bytes, void* data) = 0;
		static UniformBuffer* Create(const uint32_t& byteSize, GLenum usageHint = GL_STREAM_DRAW);
	};
