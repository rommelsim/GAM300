...
namespace AeonScripts
{
    ...
    /*
        ...
        4. Effect to last for about 5 seconds
     */
    public class LightManager : AeonBehaviour
    {
        ...
        public override void OnCreate()
       {
        ...
        public override void Update()
        {
            ...
            if (lightTimer > 0)
            {
                if (level1 != null)
                {
                    if (level1.player1respawn)
                    {

                        lightTimer -= Time.DeltaTime;
                        lightPos = new Vector3(player1.GetComponent<Transform>().Position.X, player1.GetComponent<Transform>().Position.Y + 20.0f, player1.GetComponent<Transform>().Position.Z);
                        ...
                        //respawn.Color = redLight;
                    }
                }
                if (level2 != null)
                {
                    if (level2.player1respawn2)
                    {
                        lightTimer -= Time.DeltaTime;
                        lightPos = new Vector3(player1.GetComponent<Transform>().Position.X, player1.GetComponent<Transform>().Position.Y + 20.0f, player1.GetComponent<Transform>().Position.Z);
                        entity.GetComponent<Transform>().Position = lightPos;
            ...
            else
            {
                lightStarted = false;
                if (level1 != null)
                    level1.player1respawn = false;
                if (level2 != null)
                    level2.player1respawn2 = false;
